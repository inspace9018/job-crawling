...
3. 메모장으로 열어 아래 3가지만 본인 것으로 고치기 → 저장 (`SESSION_COOKIE_DAYS=180` 줄은 그대로 둡니다)
...
1. **`.env.example`** 복사 → **`.env`** 로 이름 바꾸고, 경력·연봉 칸에 **숫자만** 채우기 (위 **「2단계」** 참고)  
...
- **`.env` 없음** → 위 **「2단계. 내 조건 입력하기」** 대로 `.env` 먼저 만들기  
